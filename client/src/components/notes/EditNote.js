...
const EditNote = () => {
  ...
  useEffect(() => {
    ...
    const getNote = async () => {
      ...
      if (id) {
        const res = await axioscall.get(`notes/${id}`, {
  ...
  const editNote = async (e) => {
    ...
    try {
      ...
      if (token) {
        ...
        await axioscall.put(`notes/${id}`, newNote, {
          headers: { Authorization: token },
        });
        ...
        <div className="mt-6 ">
          ...
          <div className="flex justify-center mt-6  ">
            <button
              ...
              className="px-4 py-2 disabled:opacity-40 text-white transition-colors duration-200 transform bg-green-500 rounded-md hover:bg-green-600 focus:outline-none focus:bg-green-600"
            >
              Create
            </button>
            <button
